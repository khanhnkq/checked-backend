...
public interface PhotoRepository extends JpaRepository<Photo, UUID> {
    ...
    @Query("SELECT p FROM Photo p WHERE p.sender.id = :userId " +
            "OR p.sender.id IN (SELECT f.friend.id FROM Friendship f WHERE f.user.id = :userId AND f.status = 'ACCEPTED') " +
            "OR p.sender.id IN (SELECT f.user.id FROM Friendship f WHERE f.friend.id = :userId AND f.status = 'ACCEPTED') " +
